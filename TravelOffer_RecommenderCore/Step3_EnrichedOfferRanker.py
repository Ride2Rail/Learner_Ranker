...
class CLASSIFIER_RESPONSE:
    ...
    def _get_GivenReqData(self,username,df_test,model_version='latest',fit_data_tag='all',normalization='zero'):
    ...
    def API_CLASSIFIER_Response(self,username,response_code,displayNum=30,checkTag=False,AutoTag =True,df_test=None):
        # try:
        if AutoTag == True:
            req = self._get_ReqData(username,response_code)
        elif AutoTag == False:
            req =self._get_GivenReqData(username,df_test)
    ...
    def API_ResultDisplay(self,username,response_code,dispayNum=30):
        ...
        df_display = df_res[['Travel Offer ID','Starting point','Destination','Via','Departure time','Arrival time','Score']]
        displayStringHead = '''=======================================================================================================
        \nDear {} : 
